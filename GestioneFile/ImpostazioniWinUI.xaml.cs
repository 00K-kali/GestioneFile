...
public partial class ImpostazioniWinUI : ContentPage
{
    string pathsfilename;
    string destpathsfilename;
    Settings settings;
    public ImpostazioniWinUI(string pathsfilename, string destpathsfilename, Settings settings)
    {
        InitializeComponent();
    ...
    async void UpdatePaths(string filepath, string path, Picker PCK)
    {
        ...
        using (StreamReader rt = new StreamReader(file))
        {
            ...
            if (!flag)
            {
                File.AppendAllText(filepath, path + Environment.NewLine);
